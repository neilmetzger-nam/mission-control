...
export default function ReviewPage() {
  ...
  return (
    <>
      <div className="mb-8">
        <h1 className="text-2xl font-bold tracking-tight mb-1">{"\u{1F504}"} Review Queue</h1>
        <p className="text-zinc-500 text-sm">{pendingItems.length} item{pendingItems.length !== 1 ? "s" : ""} awaiting review</p>
      </div>

      {pendingItems.length === 0 && resolvedItems.length === 0 && (
        <div className="bg-zinc-900 rounded-xl border border-zinc-800 p-8 text-center">
          <p className="text-zinc-500 text-sm">Queue is clear — nothing to review</p>
        </div>
      )}
